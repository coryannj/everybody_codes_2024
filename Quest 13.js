...
function platforms(input,partNo){
    ...
    while(queue.findIndex((x)=>x.length>0)!==-1){
        ...
        let nextArr = [[r,c+1],[r,c-1],[r+1,c],[r-1,c]].filter(([nr,nc])=>0<=nr && nr<lines.length && 0<=nc && nc<lines[0].length && !'# '.includes(lines[nr][nc]) && !next.includes([nr,nc].join('-')));
        ...
        if(nextArr.length>0){
            nextArr.forEach(([nr,nc])=>{
                ...
                if(minObj[nxt]===undefined||newNext[0]<minObj[nxt]){
                    ...
                    if(lines[nr][nc] !== end){
                        queue[newNext[0]].push(newNext.concat(nxt))
                    } else {
                        min = newNext[0]
                    }
                }
